...
use rmcp::model::{CallToolResponse, CallToolResult, JsonObject, ListToolsResult, Tool, ToolAnnotations};
...
pub fn get_all_tools() -> Vec<Tool> {
    ROUTES
        .iter()
        .map(|route| Tool::new(route_to_tool_name(route), route.description, build_input_schema(route)).annotate(                ToolAnnotations::new()
                    .read_only(true)
                    .destructive(false)
                    .idempotent(true)
                    .open_world(false),
...
pub async fn handle_tool_call(
    ...
    Ok(CallToolResponse::Complete(CallToolResult::success(vec![rmcp::model::ContentBlock::json(
        json_result,
    )
